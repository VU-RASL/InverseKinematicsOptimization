	...
	public class BioIKEditor : Editor {
		...
		public override void OnInspectorGUI() {
			...
			using(new EditorGUILayout.VerticalScope ("Button")) {
				...
				SetGUIColor(Color1);
				Target.SetNullspace(EditorGUILayout.Toggle("Use Nullspace", Target.GetNullspace()));
		...
		private void InspectObjective(BioObjective objective) {
			...
			using(new EditorGUILayout.VerticalScope ("Box")) {
				...
				switch(objective.GetObjectiveType()) {
					...
					InspectPose((Pose)objective);
					break;

					case ObjectiveType.PosePositionOnly:
					InspectPosePositionOnly((PosePositionOnly)objective);
					break;

					case ObjectiveType.PerpRotation:
					InspectPerpRotation((PerpRotation)objective);
					break;
		...
		private void InspectPose(Pose objective) {
			SetGUIColor(Color1);
			objective.SetRootTransform(EditorGUILayout.ObjectField("Root Transform", objective.GetRootTransform(), typeof(Transform), true) as Transform);
			SetGUIColor(Color1);
			objective.SetRootPosition(EditorGUILayout.Vector3Field("Root Position", objective.GetRootPosition()));
			SetGUIColor(Color1);
			objective.SetTargetTransform(EditorGUILayout.ObjectField("Target Transform", objective.GetTargetTransform(), typeof(Transform), true) as Transform);
			SetGUIColor(Color1);
			objective.SetTargetPosition(EditorGUILayout.Vector3Field("Target Position", objective.GetTargetPosition()));
			SetGUIColor(Color1);
			objective.SetTargetRootTransform(EditorGUILayout.ObjectField("Target Root Transform", objective.GetTargetRootTransform(), typeof(Transform), true) as Transform);
			SetGUIColor(Color1);
			objective.SetTargetRootPosition(EditorGUILayout.Vector3Field("Target Root Position", objective.GetTargetRootPosition()));
			SetGUIColor(Color1);
			objective.SetMaximumError(EditorGUILayout.DoubleField("Maximum Error", objective.GetMaximumError()));
		}

		private void InspectPosePositionOnly(PosePositionOnly objective) {
			SetGUIColor(Color1);
			objective.SetRootTransform(EditorGUILayout.ObjectField("Root Transform", objective.GetRootTransform(), typeof(Transform), true) as Transform);
			SetGUIColor(Color1);
			objective.SetRootPosition(EditorGUILayout.Vector3Field("Root Position", objective.GetRootPosition()));
		...
		private void InspectPerpRotation(PerpRotation objective) {
		...
		private void DrawObjective(BioObjective objective, bool selected) {
			...
			switch(objective.GetObjectiveType()) {
				...
				case ObjectiveType.Pose:
				DrawPose((Pose)objective);
				break;

				case ObjectiveType.PosePositionOnly:
				DrawPosePositionOnly((PosePositionOnly)objective);
				break;

				case ObjectiveType.PerpRotation:
				DrawPerpRotation((PerpRotation)objective);
				break;
		...
		private void DrawPose(Pose objective) {

		}

		private void DrawPosePositionOnly(PosePositionOnly objective) {

		}

		private void DrawPerpRotation(PerpRotation objective) {
