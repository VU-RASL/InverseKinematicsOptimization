		...
		public static BioObjective AddObjective(BioSegment segment, ObjectiveType type) {
			...
			if(Application.isPlaying) {
				switch(type) {
					...
					return (segment.gameObject.AddComponent(typeof(Pose)) as BioObjective).Create(segment);

					case ObjectiveType.PosePositionOnly:
					return (segment.gameObject.AddComponent(typeof(PosePositionOnly)) as BioObjective).Create(segment);

					case ObjectiveType.PerpRotation:
					return (segment.gameObject.AddComponent(typeof(PerpRotation)) as BioObjective).Create(segment);
				...
				switch(type) {
					...
					case ObjectiveType.Pose:
					return (Undo.AddComponent(segment.gameObject, typeof(Pose)) as BioObjective).Create(segment);

					case ObjectiveType.PosePositionOnly:
					return (Undo.AddComponent(segment.gameObject, typeof(PosePositionOnly)) as BioObjective).Create(segment);

					case ObjectiveType.PerpRotation:
					return (Undo.AddComponent(segment.gameObject, typeof(PerpRotation)) as BioObjective).Create(segment);
